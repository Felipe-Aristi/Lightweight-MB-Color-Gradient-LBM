...
namespace D3Q19
{
    ...
    __host__ __device__ constexpr real_t w() noexcept
    {
        if constexpr (s<I>() == 0)
        {
            return real_t(1.0 / 3.0);
        }

        else if constexpr (s<I>() == 1)
        {
            return real_t(1.0 / 18.0);
        }

        else
        {
            return real_t(1.0 / 36.0);
        }
    }
...
namespace LbmStencil = D3Q19;
#else
namespace LbmStencil = D3Q27;
#endif

static_assert(static_cast<int>(Q) == LbmStencil::q, "Selected stencil Q mismatch");
